...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['cid_file'])) {
    ...
    if (move_uploaded_file($_FILES['cid_file']['tmp_name'], $uploadedFile)) {
        ...
        try {
            ...
            die("<div class='error'>❌ ไม่สามารถเชื่อมต่อฐาน: " . $e->getMessage() . "</div>");
        }
        ...
        try {
            ...
            die("<div class='error'>❌ เว็บบริการไม่สามารถ: " . $e->getMessage() . "</div>");
        }
        ...
        foreach ($lines as $index => $person_id) {
            ...
            if (!preg_match('/^\d{13}$/', $person_id)) {
                echo "<div class='result warning'>⚠️ บรรทัด ".($index+1)." : รูปแบบผิด: <b>{$person_id}</b></div>";
                $invalidCount++;
                continue;
            }
            ...
            try {
                ...
                if (empty($data['Person_ID'])) {
                    echo "<div class='result info'>ℹ️ #".($index+1)." <b>{$person_id}</b> : ไม่พบข้อมูล</div>";
                    $skipCount++;
                    continue;
                }

                if (updateNHSOData($pdo, $data)) {
                    echo "<div class='result success'>✔️ #".($index+1)." <b>{$person_id}</b> : บันทึกสำเร็จ</div>";
                    $successCount++;
                } else {
                    echo "<div class='result error'>❌ #".($index+1)." <b>{$person_id}</b> : บันทึกล้มเหลว</div>";
                    $failCount++;
                }

            } catch (SoapFault $e) {
                echo "<div class='result error'>❌ #".($index+1)." <b>{$person_id}</b> : " . $e->getMessage() . "</div>";
                $failCount++;
            }
        }
        ...
        echo "<h3>📊 สรุป:</h3>";
        echo "<ul>";
        echo "<li>✔️ สำเร็จ: {$successCount}</li>";
        echo "<li>❌ ล้มเหลว: {$failCount}</li>";
        echo "<li>ℹ️ ไม่มีข้อมูลสิทธิ์: {$skipCount}</li>";
        echo "<li>⚠️ รูปแบบเลขผิด: {$invalidCount}</li>";
        echo "</ul>";
    } else {
        echo "<div class='result error'>❌ ไม่สามารถอัปโหลดไฟล์</div>";
    }
}
?>
<!DOCTYPE html>
