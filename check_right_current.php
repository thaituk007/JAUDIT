...
// ตรวจสอบจำนวน cid ใน person_checkright
$countSql = "SELECT COUNT(*) AS total FROM person_checkright";
$countResult = $mysqli->query($countSql);
$countData = $countResult->fetch_assoc();
$totalCid = $countData['total'];

// ดาวน์โหลด Excel template
if (isset($_POST['download_excel'])) {
    $sql = "SELECT idcard AS pid
            FROM person
            WHERE cid13Chk(idcard)='t'
            AND nation='99'
            AND idcard NOT IN (SELECT cid FROM person_checkright)
            LIMIT 2000";
...
// ดึงข้อมูลเพื่อแสดงในหน้าเว็บ
$sql = "SELECT idcard AS pid FROM person WHERE cid13Chk(idcard)='t' AND nation='99' AND idcard NOT IN (SELECT cid FROM person_checkright)
LIMIT 2000";
$result = $mysqli->query($sql);
$dataList = [];
...
<div class="container">
    ...
    <form action="check_right_current_update.php" method="get">
