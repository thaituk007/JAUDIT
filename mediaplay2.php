...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_FILES['file'])){
    ...
    if(!in_array($ext,$allowedExt)){ http_response_code(400); echo 'นามสกุลไฟล์ไม่รองรับ'; exit; }
    $type = mime_content_type($f['tmp_name']);
    if(!preg_match('/^(audio|video)\//',$type)){ http_response_code(400); echo 'ไฟล์ไม่ใช่ audio/video'; exit; }
...
// ================== Visualizer + Neon Glow + Peak Flash ==================
...
function drawViz(){
  ...
  if(vizMode==='bars'){
    ...
    if(peakDetected) vizCanvas.style.boxShadow='0 0 20px #7b61ff,0 0 12px #00f0ff inset';
    else vizCanvas.style.boxShadow='0 0 6px rgba(123,97,255,0.2) inset';
  }
